...
import {
  DefaultService,
  NotebookCellType,
  type BackgroundProgressCluster,
  type Cluster,
  type Notebook,
  type NotebookCell,
...
export function NotebookWorkspace({ notebookId }: NotebookWorkspaceProps) {
  ...
  const [backgroundProgress, setBackgroundProgress] = useState<BackgroundProgressCluster[]>([]);
  const [backgroundProgressLoading, setBackgroundProgressLoading] = useState(true);
  const [backgroundProgressError, setBackgroundProgressError] = useState<string | null>(null);
  ...
  const loadBackgroundProgress = useCallback(async () => {
    setBackgroundProgressLoading(true);
    setBackgroundProgressError(null);
    try {
      const response = await DefaultService.listClusterBackgroundProgress();
      setBackgroundProgress(response.clusters);
    } catch (progressError) {
      setBackgroundProgressError(getErrorMessage(progressError));
    } finally {
      setBackgroundProgressLoading(false);
    }
  }, []);

  useEffect(() => {
    void loadBackgroundProgress();
  ...
  async function runCell(cell: NotebookCell) {
    ...
    setError(null);
    try {
      const statement = cell.backgroundDdl ? `SET BACKGROUND_DDL=true;\n${cell.content}` : cell.content;
            ...
            <div className="flex flex-col gap-5">
              ...
              <Card className="border-border bg-card">
                <CardHeader className="border-b border-border/70 pb-3">
                  <CardTitle>Background DDL progress</CardTitle>
                  <CardDescription>Polling rw_ddl_progress and rw_cdc_progress across all clusters every 3 seconds.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-3 p-3 md:p-4">
                  {backgroundProgressError ? (
                    <Alert variant="destructive">
                      <AlertTitle>Progress polling failed</AlertTitle>
                      <AlertDescription>{backgroundProgressError}</AlertDescription>
                    </Alert>
                  ) : null}
                  {backgroundProgressLoading && backgroundProgress.length === 0 ? (
                    <p className="text-xs text-muted-foreground">Loading progress…</p>
                  ) : null}
                  {!backgroundProgressLoading && backgroundProgress.length === 0 ? (
                    <EmptyState description="No clusters available for background progress polling." title="No progress data" />
                  ) : null}
                  {backgroundProgress.map((clusterProgress) => (
                    <BackgroundProgressClusterCard key={clusterProgress.clusterUuid} cluster={clusterProgress} />
                  ))}
                </CardContent>
              </Card>
            </div>
...
function BackgroundProgressClusterCard({ cluster }: { cluster: BackgroundProgressCluster }) {
  return (
    <Card className="border-border/80 bg-white shadow-none">
      <CardContent className="space-y-3 p-3">
        <div>
          <div className="text-sm font-semibold">{cluster.clusterName}</div>
          <div className="text-[11px] text-muted-foreground">{cluster.clusterUuid}</div>
        </div>

        {cluster.error ? (
          <Alert variant="destructive">
            <AlertTitle>Cluster polling failed</AlertTitle>
            <AlertDescription>{cluster.error}</AlertDescription>
          </Alert>
        ) : null}

        {cluster.databases.length === 0 && !cluster.error ? (
          <p className="text-xs text-muted-foreground">No databases reported for this cluster.</p>
        ) : null}

        {cluster.databases.map((databaseProgress) => (
          <div key={`${cluster.clusterUuid}-${databaseProgress.database}`} className="space-y-2 rounded-md border border-border/80 bg-[#fbfbfd] p-2.5">
            <div className="text-xs font-semibold tracking-[0.01em] text-foreground">{databaseProgress.database}</div>
            <ProgressResultPanel result={databaseProgress.ddl} title="rw_ddl_progress" />
            <ProgressResultPanel result={databaseProgress.cdc} title="rw_cdc_progress" />
          </div>
        ))}
      </CardContent>
    </Card>
...
function ProgressResultPanel({ title, result }: { title: string; result: SqlExecutionResult }) {
  return (
    <div className="rounded-md border border-border/70 bg-white">
      <div className="border-b border-border/70 px-2.5 py-2 text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">
        {title}
      </div>
      {result.error ? (
        <div className="px-2.5 py-2 text-xs text-destructive">{result.error}</div>
      ) : (result.rows ?? []).length === 0 ? (
        <div className="px-2.5 py-2 text-xs text-muted-foreground">No active rows.</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="min-w-full border-collapse text-left text-[11px]">
            <thead className="bg-[#f7f8fc] text-muted-foreground">
              <tr>
                {(result.columns ?? []).map((column) => (
                  <th key={`${title}-${column}`} className="border-b px-2.5 py-2 font-medium">
                    {column}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {(result.rows ?? []).map((row, index) => (
                <tr key={`${title}-${index}`} className={index % 2 === 0 ? "bg-white" : "bg-[#fcfcff]"}>
                  {(row.values ?? []).map((value, valueIndex) => (
                    <td key={`${title}-${index}-${valueIndex}`} className="border-b px-2.5 py-2 align-top font-mono text-[11px] text-foreground">
                      {value ?? "NULL"}
                    </td>
                  ))}
                </tr>
